...
import { format, startOfWeek, endOfWeek, startOfMonth, endOfMonth, startOfYear, endOfYear, eachDayOfInterval, eachMonthOfInterval, addDays, addWeeks, addMonths, addYears, subDays, subWeeks, subMonths, subYears, isToday, isSameMonth, isSameDay, parseISO, isWithinInterval, getWeek, isWeekend, getDay } from "date-fns";
        ...
        delay: groupIndex * 0.05
            ...
            if (calendarItem.type === "timeoff") {
              ...
              return <DryIceOrderPreview key={`dryice-${order.id}`} order={order} side="right">
                  ...
                  duration: 0.25,
                  delay: groupIndex * 0.05 + index * 0.03
                }} onClick={e => handleDryIceOrderClick(order, e)} className="p-4 hover:bg-muted/30 cursor-pointer transition-colors flex items-center gap-4">
    ...
    return <div className="space-y-4 animate-fade-in">
        <div className={cn("p-8 rounded-2xl border backdrop-blur-sm transition-all duration-300", isToday(currentDate) ? "ring-2 ring-primary/50 bg-gradient-to-br from-primary/5 to-primary/10 shadow-lg shadow-primary/10" : "bg-card/80 hover:bg-card")}>
          ...
          {!hasItems ? <div className="text-center text-muted-foreground py-12 bg-muted/30 rounded-xl">
              ...
              }} onClick={e => handleDryIceOrderClick(order, e)} className={cn("p-4 rounded-xl text-sm transition-all duration-300 hover:scale-[1.02] hover:shadow-md cursor-pointer", getDryIceStatusColor(order.status))}>
                        <div className="flex items-center justify-between gap-3">
                          <div className="flex items-center gap-3">
            ...
            animationDelay: `${index * 30}ms`
                ...
                if (entry.type === 'timeoff') {
                  ...
                  return <DryIceOrderPreview key={order.id} order={order} side="bottom" align="start">
                          <div onClick={e => handleDryIceOrderClick(order, e)} className={cn("text-xs px-2 py-1.5 rounded-lg truncate flex items-center gap-1.5 transition-transform hover:scale-105 cursor-pointer border border-cyan-500/30 bg-muted-foreground text-destructive-foreground")}>
                ...
                return null;
                  ...
                  {allItems.length > 3 && <div className="text-xs text-primary font-medium pl-1">
        ...
        <div className="grid grid-cols-7 gap-1.5">
          ...
          return <div key={day.toISOString()} onClick={e => isCurrentMonth && handleDayClick(day, e)} className={cn("min-h-[90px] p-2 rounded-xl border transition-all duration-200", isCurrentMonth ? isWeekendDay ? "bg-primary/5 backdrop-blur-sm border-primary/20 hover:bg-primary/10 hover:shadow-sm" : "bg-card/80 backdrop-blur-sm border-border/50 hover:bg-card hover:shadow-sm" : "bg-muted/20 border-transparent opacity-50", isCurrentDay && "ring-2 ring-primary/50 bg-gradient-to-br from-primary/10 to-transparent", isDragOver && "ring-2 ring-blue-500 bg-blue-50/50 dark:bg-blue-950/30 scale-[1.02]", isCurrentMonth && isAdmin && "cursor-pointer")} onDragOver={e => handleDragOver(e, day)} onDragLeave={handleDragLeave} onDrop={e => handleDrop(e, day)}>
                ...
                if (entry.type === 'timeoff') {
                  ...
                  const order = entry.item as DryIceOrderWithDetails;
                  return <DryIceOrderPreview key={order.id} order={order} side="bottom" align="start">
                          <div onClick={e => handleDryIceOrderClick(order, e)} className="text-[10px] px-1.5 py-1 rounded-md truncate flex items-center gap-1 transition-transform hover:scale-105 cursor-pointer bg-cyan-500/20 text-cyan-700 dark:text-cyan-300 border border-cyan-500/30">
                ...
                return null;
                  ...
                  {allItems.length > 2 && <div className="text-[10px] text-primary font-semibold pl-1">
                      +{allItems.length - 2}
                    </div>}
                </div>
              </div>;
