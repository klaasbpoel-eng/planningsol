...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const dropAndRecreateSql = `
      ...
      CREATE POLICY "Admins can view all dry ice orders" ON public.dry_ice_orders FOR SELECT USING (is_admin());
      CREATE POLICY "Admins can create dry ice orders" ON public.dry_ice_orders FOR INSERT WITH CHECK (is_admin());
      CREATE POLICY "Admins can update dry ice orders" ON public.dry_ice_orders FOR UPDATE USING (is_admin());
      CREATE POLICY "Admins can delete dry ice orders" ON public.dry_ice_orders FOR DELETE USING (is_admin());

      CREATE POLICY "Users can view assigned or created dry ice orders" ON public.dry_ice_orders FOR SELECT
      ...
      CREATE POLICY "Supervisors can view dry ice orders at their location" ON public.dry_ice_orders FOR SELECT
      ...
      CREATE POLICY "Operators can view dry ice orders at their location" ON public.dry_ice_orders FOR SELECT
      ...
      CREATE TRIGGER update_dry_ice_orders_updated_at BEFORE UPDATE ON public.dry_ice_orders FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
    `;
    ...
    return new Response(
      JSON.stringify({ success: true, message: "Tabel dry_ice_orders is succesvol verwijderd en opnieuw aangemaakt" }),
